    ...
    public class Program
    {
        ...
        public static void init_employees()//מילוי מערך עובדים מתוך בסיס הנתונים
        {
            ...
            while (rdr.Read())
            {
                ...
                Employee employee = new Employee(rdr.GetValue(0).ToString(), rdr.GetValue(1).ToString(), rdr.GetValue(2).ToString(), rdr.GetValue(3).ToString(), employeeType , true , false);
        ...
        public static void init_leads()//מילוי מערך לקוחות פוטנציאלים מתוך בסיס הנתונים
        {
            ...
            while (rdr.Read())
            {
                ...
                rdr.GetValue(3).ToString(), DateTime.Parse((rdr.GetValue(4).ToString())), rdr.GetValue(5) , leadStatus
                , leadSource, seekEemploye(rdr.GetValue(8).ToString()) ) ; 
                Lead.Add (lead); 
